...
# Linear module for next clip embedding prediction
class ModelLinear(nn.Module):
    def __init__(self):
        super(ModelLinear, self).__init__()
        lin = torch.nn.Linear(1024, 1024, bias=False)
        lin.weight.data.copy_(torch.eye(1024))
...
class WARHOL(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.clip_img_proj = nn.Linear(512, dim)
        ...
        self.condition_pos_emb = nn.Embedding(2, dim)
    ...
    def generate_texts(
        ...
        *,
        ...
        if self.use_next_prod_module:
            clip_img, clip_txt = self.next_clip_product(clip_img, clip_txt)
        
        # Conditioning tokens
        clip_img_token = self.clip_img_proj(clip_img)
        clip_txt_token = self.clip_txt_proj(clip_txt)
        condition_tokens = torch.cat((clip_img_token, clip_txt_token), dim=1)
        condition_tokens += self.condition_pos_emb(torch.arange(2, device=device))
    ...
    def generate_images(
        ...
        *,
        ...
        for cur_len in range(out.shape[1], total_len):
            ...
            logits = self(clip_img, clip_txt, text, image=image, mask = mask)[:, -1, :]
    ...
    def forward(
        ...
        if self.use_next_prod_module:
            ...
            if self.use_neg_samples and return_loss:
                clip_losses = self.get_clip_losses(fut_clip, torch.cat((clip_img, clip_txt), dim=-1).squeeze(1), neg_clips)
        # Conditioning tokens
        clip_img_token = self.clip_img_proj(clip_img)
        clip_txt_token = self.clip_txt_proj(clip_txt)
        condition_tokens = torch.cat((clip_img_token, clip_txt_token), dim=1)
        ...
        if tokens.shape[1] > total_seq_len + 1: # there are two prepended cond tokens
            seq_len -= 1
            tokens = tokens[:, :-1]

        out = self.transformer(tokens) 
        out = out[:, 1:] # remove first output token which corresponds to the second input cond token
        seq_len -= 1
    ...
    def next_clip_product(self, clip_img, clip_txt):
        past_product = torch.cat((clip_img, clip_txt), dim=-1)
        next_product = self.next_product_proj(past_product)
        new_clip_img = next_product[:, :, :512]
        new_clip_txt = next_product[:, :, 512:] 
        new_clip_img = new_clip_img / new_clip_img.norm(dim=-1, keepdim=True)
        new_clip_txt = new_clip_img / new_clip_txt.norm(dim=-1, keepdim=True)
        return new_clip_img, new_clip_txt
